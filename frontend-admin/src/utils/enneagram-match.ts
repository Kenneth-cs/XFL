...
const MALE_TO_FEMALE_MATRIX: number[][] = [
  ...
  [0, 0, 1, 0, 1, 0, -1, 1, 1], // 男1  -> 1-7不匹配; +1:女3,5,8,9
  [0, 0, 0, 1, 1, 1, 0, 1, -1], // 男2  -> 2-9不匹配; +1:女4,5,6,8
  [1, 0, 0, 0, 0, -1, 1, 1, 1], // 男3  -> 3-6不匹配; +1:女1,7,8,9
  [0, 1, 0, -1, 1, 1, 1, -1, 1], // 男4  -> 4-4不匹配, 4-8不匹配; +1:女2,5,6,7,9
  [1, 1, 0, 1, 1, 0, 0, 1, -1], // 男5  -> 5-9不匹配; +1:女1,2,4,5,8
  [0, 1, -1, 1, 0, 1, 0, -1, 1], // 男6  -> 6-3不匹配, 6-8不匹配; +1:女2,4,6,9
  [-1, 0, 0, 1, 0, 0, 0, 0, 1], // 男7  -> 7-1不匹配; +1:女4,9
  [1, 1, 1, -1, 1, -1, 0, -1, 1], // 男8  -> 8-4不匹配, 8-6不匹配, 8-8不匹配; +1:女1,2,3,5,9
  [1, -1, 1, 1, -1, 1, 1, 0, 0], // 男9  -> 9-2不匹配, 9-5不匹配; +1:女1,3,4,6,7
]

// 女性到男性的匹配矩阵（与男性矩阵存在非对称差异，不可简单转置）
// 行索引: 女性类型 (1-9), 列索引: 男性类型 (1-9)
const FEMALE_TO_MALE_MATRIX: number[][] = [
  // 男1  男2  男3  男4  男5  男6  男7  男8  男9
  [0, 0, 1, 0, 1, 0, -1, 1, 1], // 女1  -> 1-7不匹配; +1:男3,5,8,9
  [0, 0, 0, 1, 1, 1, 0, 1, -1], // 女2  -> 2-9不匹配; +1:男4,5,6,8
  [1, 0, 0, 0, 0, -1, 1, 1, 1], // 女3  -> 3-6不匹配; +1:男1,7,8,9
  [0, 1, 0, -1, 1, 1, 1, -1, 1], // 女4  -> 4-4不匹配, 4-8不匹配; +1:男2,5,6,7,9
  [1, 1, 0, 1, 1, 0, 0, 1, -1], // 女5  -> 5-9不匹配; +1:男1,2,4,5,8
  [0, 1, -1, 1, 0, 1, 0, -1, 1], // 女6  -> 6-3不匹配, 6-8不匹配; +1:男2,4,6,9
  [-1, 0, 0, 1, 0, 0, 0, 0, 1], // 女7  -> 7-1不匹配; +1:男4,9
  [1, 1, 1, -1, 1, -1, 0, -1, 1], // 女8  -> 8-4不匹配, 8-6不匹配, 8-8不匹配; +1:男1,2,3,5,9
  [1, -1, 1, 1, -1, 1, 1, 0, 0], // 女9  -> 9-2不匹配, 9-5不匹配; +1:男1,3,4,6,7
]
...
export function calculateEnneagramMatch(
  ...
  // 遍历用户的Top3类型,累加匹配得分
  const matrix = gender === '男' ? MALE_TO_FEMALE_MATRIX : FEMALE_TO_MALE_MATRIX
