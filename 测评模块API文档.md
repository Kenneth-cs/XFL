# 幸福力测评模块 API 文档

## 📦 模块结构

```
backend/src/modules/assessment/
├── assessment.module.ts              # 模块定义
├── assessment.controller.ts          # API控制器
├── assessment.service.ts             # 业务逻辑层
├── dto/
│   ├── submit-enneagram.dto.ts      # 九型人格提交DTO
│   ├── submit-attachment.dto.ts     # 依恋关系提交DTO
│   └── submit-happiness.dto.ts      # 幸福力提交DTO
└── calculators/
    ├── enneagram.calculator.ts      # 九型人格计算器
    ├── attachment.calculator.ts     # 依恋关系计算器
    └── happiness.calculator.ts      # 幸福力计算器
```

---

## 🌐 API 接口列表

### 基础路径
所有接口统一前缀：`/api/v1/assessments`

---

## 1. 九型人格测评

### 1.1 获取题目
**请求**
```http
GET /api/v1/assessments/enneagram/questions
Authorization: Bearer <token>
```

**响应**
```json
{
  "total": 144,
  "questions": [
    {
      "questionId": 1,
      "content": "以下两个选项我比较倾向于",
      "options": [
        { "text": "我浪漫并富于幻想", "type": 5 },
        { "text": "我很实际并实事求是", "type": 2 }
      ]
    }
  ]
}
```

### 1.2 提交测评
**请求**
```http
POST /api/v1/assessments/enneagram/submit
Authorization: Bearer <token>
Content-Type: application/json

{
  "userId": "XFL00100001",
  "answers": [
    { "questionId": 1, "selectedType": 5 },
    { "questionId": 2, "selectedType": 1 },
    ...
  ]
}
```

**响应**
```json
{
  "recordId": 123,
  "result": {
    "top3": [5, 2, 4],
    "percentages": {
      "1": 0.45,
      "2": 0.78,
      "3": 0.52,
      "4": 0.72,
      "5": 0.85,
      ...
    },
    "rawScores": {
      "1": 12,
      "2": 20,
      ...
    },
    "primaryType": 5
  }
}
```

**计算逻辑**：
1. 统计每个人格类型（1-9号）的实际得分
2. 计算百分比 = 实际得分 / 标准满分
3. 按百分比降序排序，获取 Top 3
4. 有效阈值筛选：`(最高分 - 15%)` 作为阈值，至少保留3个

---

## 2. 依恋关系测评

### 2.1 获取题目
**请求**
```http
GET /api/v1/assessments/attachment/questions
Authorization: Bearer <token>
```

**响应**
```json
{
  "total": 36,
  "questions": [
    {
      "questionId": "A1",
      "content": "在恋爱中特别渴望亲密",
      "dimension": "A"
    },
    {
      "questionId": "B1",
      "content": "重视自我独立，不愿意过度投入感情",
      "dimension": "B"
    },
    {
      "questionId": "C1",
      "content": "言行一致，值得信赖",
      "dimension": "C"
    }
  ]
}
```

### 2.2 提交测评
**请求**
```http
POST /api/v1/assessments/attachment/submit
Authorization: Bearer <token>
Content-Type: application/json

{
  "userId": "XFL00100001",
  "selectedQuestions": ["A1", "A5", "B3", "C10"]
}
```

**响应**
```json
{
  "recordId": 124,
  "result": {
    "anxietyScore": 2,
    "avoidanceScore": 1,
    "securityScore": 1,
    "type": "得分不足",
    "typeLabel": "得分不足",
    "description": "三个维度得分都低于5分，无法给出具体的依恋类型"
  }
}
```

**计算逻辑**：
- 焦虑得分（A）= 选中A开头题目的数量
- 回避得分（B）= 选中B开头题目的数量
- 安全感得分（C）= 选中C开头题目的数量
- 判定优先级：紊乱型 (A≥5 & B≥5) > 焦虑型 (A≥5) > 回避型 (B≥5) > 安全型 (C≥5)

---

## 3. 婚恋幸福力测评

### 3.1 获取题目
**请求**
```http
GET /api/v1/assessments/happiness/questions
Authorization: Bearer <token>
```

**响应**
```json
{
  "total": 155,
  "questions": [
    {
      "questionId": "D1-Q1",
      "dimensionId": 1,
      "subLabel": "创造力",
      "content": "我喜欢尝试新方法来解决问题",
      "options": [
        { "value": 1, "label": "非常不符合" },
        { "value": 2, "label": "不太符合" },
        { "value": 3, "label": "一般" },
        { "value": 4, "label": "比较符合" },
        { "value": 5, "label": "非常符合" }
      ],
      "isIgnored": false
    }
  ]
}
```

### 3.2 提交测评
**请求**
```http
POST /api/v1/assessments/happiness/submit
Authorization: Bearer <token>
Content-Type: application/json

{
  "userId": "XFL00100001",
  "answers": [
    { "questionId": "D1-Q1", "score": 4 },
    { "questionId": "D1-Q2", "score": 5 },
    ...
  ]
}
```

**响应**
```json
{
  "recordId": 125,
  "result": {
    "dimensions": [
      {
        "dimensionId": 1,
        "dimensionName": "积极性格",
        "shortName": "乐商",
        "rawScore": 90,
        "maxScore": 120,
        "targetScore": 10,
        "normalizedScore": 7.5,
        "percentage": 0.75
      },
      ...
    ],
    "averageScore": 7.25,
    "totalRawScore": 850,
    "totalMaxScore": 1200
  }
}
```

**计算逻辑**：
1. **统一计分规则**：所有题目选项value直接计分（1-5分对应1-5分），无反向计分
2. 按维度分组统计原始得分
3. 归一化得分 = (原始得分 / 最高分) × 目标分（通常10分）
4. 百分比 = 原始得分 / 最高分
5. 平均分 = 所有维度归一化得分的平均值

**20个维度**：
1. 积极性格（乐商）- 24题
2. 积极情绪（情商）- 4题
3. 积极心态（心商）- 3题
4. 积极力量（施容）- 4题
5. 积极语言（语商）- 3题
6. 积极沟通（施语）- 3题
7. 积极教育（素质）- 5题
8. 积极教养（管教）- 5题
9. 积极习惯（自律）- 5题
10. 积极天赋（优势）- 2题
11. 积极自尊（修养）- 10题
12. 积极关系（爱商）- 23题
13. 积极改变（勇气）- 5题
14. 积极信念（希望）- 12题（含4题干扰题不计分）
15. 积极体验（福流）- 9题
16. 积极品质（福商）- 10题
17. 积极投入（志商）- 3题
18. 积极自我（德商）- 10题
19. 积极目标（财商）- 4题
20. 积极意义（健商）- 12题

---

## 4. 测评历史查询

### 4.1 获取用户测评历史
**请求**
```http
GET /api/v1/assessments/history/:userId
Authorization: Bearer <token>
```

**响应**
```json
[
  {
    "id": 125,
    "type": 3,
    "typeName": "婚恋幸福力",
    "isLatest": true,
    "createdAt": "2026-01-12T08:30:00.000Z",
    "resultData": { ... }
  },
  {
    "id": 124,
    "type": 2,
    "typeName": "依恋关系",
    "isLatest": true,
    "createdAt": "2026-01-12T08:00:00.000Z",
    "resultData": { ... }
  }
]
```

### 4.2 获取用户最新测评结果
**请求**
```http
GET /api/v1/assessments/latest/:userId
Authorization: Bearer <token>
```

**响应**
```json
{
  "enneagram": {
    "recordId": 123,
    "result": { ... },
    "createdAt": "2026-01-11T10:00:00.000Z"
  },
  "attachment": {
    "recordId": 124,
    "result": { ... },
    "createdAt": "2026-01-12T08:00:00.000Z"
  },
  "happiness": {
    "recordId": 125,
    "result": { ... },
    "createdAt": "2026-01-12T08:30:00.000Z"
  }
}
```

---

## 🔒 认证说明

所有接口均需要携带JWT Token：
```http
Authorization: Bearer <your_jwt_token>
```

---

## 📊 数据库存储

### `assessment_record` 表结构
| 字段名 | 类型 | 说明 |
|--------|------|------|
| id | BIGINT | 主键 |
| user_id | VARCHAR(20) | 用户ID |
| type | TINYINT | 1:九型, 2:依恋, 3:幸福力 |
| answers | JSON | 原始答卷数据 |
| result_data | JSON | 结构化结果 |
| raw_scores | JSON | 原始分数（调试用） |
| is_latest | TINYINT | 1:最新, 0:历史 |
| created_at | DATETIME | 创建时间 |
| updated_at | DATETIME | 更新时间 |

---

## 🎯 核心特性

### ✅ 已实现
1. **题库代码化**：所有题目存储在TypeScript常量中，零数据库查询
2. **前后端分离**：题目内容暴露前端，计算逻辑保护后端
3. **统一计分规则**：幸福力测评无反向计分，直接按选项value计分
4. **结果持久化**：所有测评结果存入数据库，支持历史查询
5. **版本控制**：自动标记最新记录，旧记录保留为历史

### 🔐 安全性
- 题目内容：前端可见（用户需要看到题目）
- 计算逻辑：严格保护在后端（核心竞争力）
- 匹配算法：完全后端实现（不暴露任何规则）

---

## 📝 开发规范

### TypeScript接口定义
- 所有接口均有类型定义
- 使用`class-validator`进行参数校验
- 使用`class-transformer`进行类型转换

### 日志记录
- 关键操作自动记录日志
- 包含用户ID、操作类型、结果摘要

### 错误处理
- 统一使用NestJS内置异常
- 前端可获得清晰的错误提示

---

## 🚀 后续扩展

### 可选功能
1. **测评报告生成**：根据结果生成PDF报告
2. **数据统计分析**：门店维度的测评数据统计
3. **结果对比**：同一用户多次测评结果对比
4. **权限控制**：红娘查看会员测评结果的权限管理

---

**创建时间**：2026-01-12  
**版本**：v1.0  
**作者**：AI Assistant

