...
| created_by | VARCHAR(20) | 创建人（系统用户ID，记录哪个管理员/红娘发起的匹配）|
...
#### 问题背景
在实际开发中发现，TypeORM（以及部分其他 ORM 框架）在处理 **"共享主键的一对一关系"** 时存在已知缺陷。

**数据库设计特点**：
- `app_user` 表：主键为 `id`
- `app_user_profile` 表：主键为 `user_id`，同时 `user_id` 也是外键指向 `app_user.id`
- 这种设计在数据库层面是合理的（节省存储空间、保证数据一致性）

**问题表现**：
```typescript
// ❌ 这种写法在共享主键场景下经常失败
const users = await this.appUserRepository.find({
  relations: ['profile']  // profile 关系无法正确加载
});
// 结果：users[0].profile === undefined
```

虽然 Entity 定义正确、数据库中数据完整，但 ORM 自动关系映射会失效，导致前端收不到 profile 数据。

#### 解决方案：手动查询 + 拼装

**推荐做法**（已在 `user.service.ts` 中实施）：

```typescript
// ✅ 正确的实现方式
async findAllAppUsers(storeId?: string, page = 1, limit = 20) {
  // 1. 先查询用户列表（不使用 relations）
  ...
  // 5. 绕过序列化拦截器（防止 ClassSerializerInterceptor 过滤掉 profile）
...
1. **避免使用 `relations: ['profile']`**：在共享主键场景下不可靠。
2. **使用 `findByIds` 批量查询**：性能优于逐个查询（单次 SQL，利用 IN 语句）。
3. **Map 结构快速映射**：时间复杂度 O(n)，高效拼装。
4. **JSON 序列化绕过元数据**：`JSON.parse(JSON.stringify())` 可移除 TypeORM 实体的类元数据，确保数据能正常序列化给前端（避免被 `ClassSerializerInterceptor` 过滤）。

#### 影响范围

此问题影响所有需要返回"用户+档案"完整数据的接口，包括：
- ✅ **用户列表查询** (`GET /users/app`)：已修复
- ✅ **智能匹配模块** (`POST /matches/initiate`)：候选人列表需包含档案信息
- ⚠️ **其他可能受影响的接口**：任何涉及 `app_user` 关联 `app_user_profile` 的查询，建议统一采用手动拼装方案

#### 最佳实践总结

**当遇到以下情况时，应采用手动查询 + 拼装方案：**
1. 使用了共享主键的一对一关系（主键即外键）
2. ORM 自动关系加载出现不稳定行为
3. 需要对关联数据进行复杂的后处理（如权限过滤、脱敏）
...
**权衡建议**：对于核心业务接口（如用户档案、匹配引擎），应优先保证稳定性和可控性，手动拼装是更优选择。
