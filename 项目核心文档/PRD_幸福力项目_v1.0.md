...
*   **档案详情 (动态表单)**：
    ...
    *   **测评结果**：
        *   **九型人格**：展示各人格百分比柱状图（降序）、Top3人格标签、与异性的性格匹配结论（包含匹配度、互斥预警）。
        *   **依恋关系**：展示类型标签（如“安全型”或“得分不足”）、三维度（焦虑/回避/安全）得分进度条、核心特质描述。
            *   *特殊规则*：若三维度得分均低于5分，显示“得分不足”及对应解释文案。
        *   **婚恋幸福力**：
            *   **双环图对比**：左侧展示用户实得分子维度圆环，右侧展示满分参考圆环。
            *   **文字分析**：根据得分生成高分模块（6-10分）和低分模块（≤6分）的针对性建议文案。
            *   **总分**：不直接显示总分，重点展示维度分布与建议。
    *   **MV值计算**：
        *   **必填项标记**：前端通过 `*` 标示参与MV计算的必填字段。
        *   **实时反馈**：输入字段后实时计算，并在字段旁以绿色文字显示该项得分（如：`身高: 175 (得分: 10分)`）。
        *   **总分展示**：显眼展示MV总分，支持手动“重新计算”。
    *   **服务轨迹 (三大轨迹)**：
...
*   **计算因子**：
    ...
    *   **女方**：年龄、BMI（身高体重推算）、长相、罩杯、学历、**性格（九型匹配度）**、家庭环境。
        *   *性格维度计算规则*：基于九型人格测评结果，统计该用户Top3人格能匹配（相性+1）的异性人格类型数量。
        *   *评分标准*：匹配1个=4.5分，2个=6.5分，3个=8.5分，4个=10.5分，5个及以上=12.5分。若未测评则得0分。
