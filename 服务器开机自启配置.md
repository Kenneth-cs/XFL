# 服务器开机自启配置说明

## ✅ 已配置的服务

### 1. PM2（后端Node.js服务）
- **状态**：✅ 已启用
- **服务名**：`pm2-ubuntu.service`
- **管理的进程**：`nest-backend`（幸福力后端服务，端口3001）
- **配置文件**：`/etc/systemd/system/pm2-ubuntu.service`

**管理命令：**
```bash
# 查看PM2服务状态
sudo systemctl status pm2-ubuntu

# 启动/停止/重启PM2服务
sudo systemctl start pm2-ubuntu
sudo systemctl stop pm2-ubuntu
sudo systemctl restart pm2-ubuntu

# PM2进程管理
pm2 list              # 查看所有进程
pm2 restart nest-backend  # 重启后端服务
pm2 logs nest-backend     # 查看日志
pm2 save              # 保存进程列表（添加新服务后必须执行）
```

---

### 2. Nginx（前端Web服务器）
- **状态**：✅ 已启用
- **服务名**：`nginx.service`
- **功能**：
  - 反向代理后端API（`/api` → `http://localhost:3001`）
  - 静态文件服务（H5前端：`/h5`，Admin后台：`/admin`）

**管理命令：**
```bash
# 查看Nginx状态
sudo systemctl status nginx

# 启动/停止/重启Nginx
sudo systemctl start nginx
sudo systemctl stop nginx
sudo systemctl restart nginx

# 重新加载配置（不中断服务）
sudo nginx -t          # 测试配置是否正确
sudo nginx -s reload   # 重新加载配置
```

**配置文件**：`/etc/nginx/sites-available/xinfu.conf`

---

### 3. MySQL（数据库服务）
- **状态**：✅ 已启用
- **服务名**：`mysql.service`
- **数据库名**：`xinfu_db`
- **用户**：`root`
- **密码**：`Xinfuli2024!`

**管理命令：**
```bash
# 查看MySQL状态
sudo systemctl status mysql

# 启动/停止/重启MySQL
sudo systemctl start mysql
sudo systemctl stop mysql
sudo systemctl restart mysql

# 连接数据库
mysql -uroot -p'Xinfuli2024!' xinfu_db
```

---

### 4. Redis（缓存服务）
- **状态**：✅ 已启用
- **服务名**：`redis-server.service`
- **端口**：6379
- **用途**：ID生成器原子计数、缓存

**管理命令：**
```bash
# 查看Redis状态
sudo systemctl status redis-server

# 启动/停止/重启Redis
sudo systemctl start redis-server
sudo systemctl stop redis-server
sudo systemctl restart redis-server
```

---

## 🔄 开机启动流程

服务器重启后，服务启动顺序：

1. **MySQL** 启动（数据库服务）
2. **Redis** 启动（缓存服务）
3. **Nginx** 启动（Web服务器）
4. **PM2** 启动（进程管理器）
5. **nest-backend** 自动启动（后端服务）

---

## 🧪 测试开机自启

### 方法1：查看服务状态
```bash
ssh -i ~/.ssh/XFL.pem ubuntu@124.222.88.25

# 检查所有服务是否启用
sudo systemctl is-enabled pm2-ubuntu
sudo systemctl is-enabled nginx
sudo systemctl is-enabled mysql

# 查看运行状态
pm2 list
sudo systemctl status nginx
sudo systemctl status mysql
```

### 方法2：模拟重启（可选，谨慎操作）
```bash
# 重启服务器（会中断服务1-2分钟）
ssh -i ~/.ssh/XFL.pem ubuntu@124.222.88.25 "sudo reboot"

# 等待2分钟后，检查服务是否自动启动
ssh -i ~/.ssh/XFL.pem ubuntu@124.222.88.25 "pm2 list && sudo systemctl status nginx mysql"

# 访问前端验证
# http://124.222.88.25:8080/admin/
# http://124.222.88.25:8080/h5/
```

---

## 📋 访问地址

- **Admin后台**：http://124.222.88.25:8080/admin/
- **H5前台**：http://124.222.88.25:8080/h5/
- **API接口**：http://124.222.88.25:8080/api/v1/

---

## ⚠️ 故障排查

### 如果重启后服务未启动：

1. **检查PM2服务**
```bash
sudo systemctl status pm2-ubuntu
sudo journalctl -u pm2-ubuntu -n 50  # 查看日志
```

2. **检查后端进程**
```bash
pm2 list
pm2 logs xinfu-backend --lines 50
```

3. **检查端口占用**
```bash
sudo netstat -tlnp | grep -E ':(3001|8080|3306)'
```

4. **手动启动服务**
```bash
# 如果PM2服务未启动
sudo systemctl start pm2-ubuntu

# 如果后端进程未运行
cd /www/projects/xinfu-project/backend
pm2 start ecosystem.config.js
pm2 save
```

---

## 🔐 安全建议

1. **定期备份数据库**
```bash
# 备份脚本（可以添加到crontab定时执行）
mysqldump -uroot -p'Xinfuli2024!' xinfu_db > /backup/xinfu_db_$(date +%Y%m%d).sql
```

2. **监控服务状态**
```bash
# 添加监控脚本，每5分钟检查一次
crontab -e
# 添加：*/5 * * * * /path/to/check_services.sh
```

3. **保持系统更新**
```bash
sudo apt update && sudo apt upgrade -y
```

---

## 📞 联系方式

如有问题，请检查：
1. PM2日志：`pm2 logs xinfu-backend`
2. Nginx日志：`/var/log/nginx/error.log`
3. MySQL日志：`/var/log/mysql/error.log`
